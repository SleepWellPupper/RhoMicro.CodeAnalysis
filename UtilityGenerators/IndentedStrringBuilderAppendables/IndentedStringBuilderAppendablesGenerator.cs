...
/// <summary>
/// Generates appendables for operator-based use of the IndentedStringBuilder.
/// </summary>
[Generator(LanguageNames.CSharp)]
[NonEquatable]
public sealed partial class IndentedStringBuilderAppendablesGenerator : IIncrementalGenerator
{
    /// <inheritdoc/>
    ...
    private static String FinalStep(ImmutableHashSet<Signature> signatures, CancellationToken ct)
    {
        ...
        var builder = new IndentedStringBuilder(IndentedStringBuilderOptions.GeneratedFile with
        {
        ...
        foreach(var (name, typeParameters, parameters, constraints, arguments) in signatures)
        {
            ...
            _ = ( parameters == String.Empty && constraints.Count == 0 ? fieldsBuilder : builder ).AppendLine()
                .Append("new")
                .OpenBlock(Blocks.Parentheses with { ClosingDelimiter = ");\n" })
                    .Append("b => ")
    ...
    private static Boolean Predicate(SyntaxNode node, CancellationToken ct)
    {
        ...
        return tds.Modifiers.Any(m => m.Text == "partial");
    }
}
