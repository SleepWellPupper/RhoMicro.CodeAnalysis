...
`whitespace` tokens are recognized as trivia if they are the only text preceding an `open-code-block` or `open-template-block` token.
A `newline` token is recognized as trivia if it is directly following a `close-code-block` or `close-template-block` token.

For now we don't want to do any fancy indentation recognition other than the following (Note that this functionality is not implemented yet.):

- subtemplate contents are treated like raw string literals;
  i.e. each line is trimmed of leading whitespace of length
  up to `<:` token:
...
This produces an error diagnostic:
```
   <:
 foo
     bar
